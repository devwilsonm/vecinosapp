...
const seed = db.transaction(async () => {
  ...
  await db.prepare("DELETE FROM receipt_allocations").run();
  await db.prepare("DELETE FROM receipts").run();
  await db.prepare("DELETE FROM occupants").run();
  await db.prepare("DELETE FROM buildings").run();
  ...
  const occupantIds = [];
  for (const row of occupants) occupantIds.push((await insertOccupant.run(...row)).lastInsertRowid);
  ...
  const insertReceipt = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pendiente')
  `);
  const waterId = (await insertReceipt.run(buildingId, "agua", "AG-2026-001", "enero 2026", "2026-01-03", "2026-01-20", 30000, "Recibo de agua del edificio.")).lastInsertRowid;
  const electricityId = (await insertReceipt.run(buildingId, "luz", "LU-2026-001", "enero 2026", "2026-01-05", "2026-01-22", 42000, "Recibo de luz de áreas comunes.")).lastInsertRowid;
  ...
  `);
  const allocationIds = [];
  for (const [index, amount] of splitAmount(30000, occupantIds.length).entries()) {
    allocationIds.push((await insertAllocation.run(waterId, occupantIds[index], amount, amount)).lastInsertRowid);
  }
  ...
  const insertPayment = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?)
  `);
  await insertPayment.run(allocationIds[0], 5000, "2026-01-10", "yape/plin", "Pago completo");
