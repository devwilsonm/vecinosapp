...
const postgresSchema = `
  CREATE TABLE IF NOT EXISTS buildings (
    ...
    public_link_ttl_hours INTEGER NOT NULL DEFAULT 24 CHECK (public_link_ttl_hours BETWEEN 1 AND 48),
  ...
  CREATE TABLE IF NOT EXISTS public_report_links (
    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    token TEXT NOT NULL UNIQUE, from_month TEXT NOT NULL, to_month TEXT NOT NULL,
    service_type TEXT NOT NULL DEFAULT 'all', building_ids TEXT NOT NULL DEFAULT '[]',
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, expires_at TIMESTAMP
  );
  ...
  CREATE INDEX IF NOT EXISTS idx_public_report_links_token ON public_report_links(token);
...
async function initDb() {
  ...
  try {
    await db.exec("ALTER TABLE buildings ADD COLUMN public_link_ttl_hours INTEGER NOT NULL DEFAULT 24 CHECK (public_link_ttl_hours BETWEEN 1 AND 48)");
  } catch (error) {
    const message = String(error?.message || error).toLowerCase();
    if (!message.includes("already exists") && !message.includes("duplicate column")) throw error;
  }
